...
[il,jl,kl] = meshgrid([0,nelx], 0, 0:nelz);                       % Coordinates 懸臂樑: [il,jl,kl] = meshgrid([0,nelx], 0, 0:nelz); 簡直樑: il = nelx/2; jl = nely; kl = nelz/2; FCC: il = [nelx/2 nelx nelx/2 0 nelx/2 nelx/2]; jl = [nely nely/2 nely/2 nely/2 nely/2 0]; kl = [nelz/2 nelz/2 nelz nelz/2 0 nelz/2];
loadnid = kl*(nelx+1)*(nely+1)+il*(nely+1)+(nely+1-jl); % Node IDs 懸臂樑: loadnid = kl*(nelx+1)*(nely+1)+il*(nely+1)+(nely+1-jl);
loaddof = 3*loadnid(:) - 1;                             % DOFs 懸臂樑: loaddof = 3*loadnid(:) - 1;
% USER-DEFINED SUPPORT FIXED DOFs
[iif,jf,kf] = meshgrid(0,0:nely,0:nelz);   % Coordinates 懸臂樑: [iif,jf,kf] = meshgrid(0,0:nely,0:nelz); 簡直樑: iif = [0 0 nelx nelx]; jf = [0 0 0 0]; kf = [0 nelz 0 nelz];
...
axis equal; axis tight; box on; view([30,30]); pause(1e-6); % axis off; 
end
% =========================================================================
